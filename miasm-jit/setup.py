...
def make_ext_modules():
    vm_common = [
        source_path("miasm/jitter/vm_mngr.c"),
        source_path("miasm/jitter/vm_mngr_py.c"),
        source_path("miasm/jitter/bn.c"),
    ]
    core_common = [
        source_path("miasm/jitter/JitCore.c"),
        *vm_common,
        source_path("miasm/jitter/op_semantics.c"),
    ]

    return [
        Extension("miasm.jitter.VmMngr", vm_common),
        Extension(
            "miasm.jitter.arch.JitCore_x86",
            [*core_common, source_path("miasm/jitter/arch/JitCore_x86.c")],
        ),
        Extension(
            "miasm.jitter.arch.JitCore_arm",
            [*core_common, source_path("miasm/jitter/arch/JitCore_arm.c")],
        ),
        Extension(
            "miasm.jitter.arch.JitCore_aarch64",
            [*core_common, source_path("miasm/jitter/arch/JitCore_aarch64.c")],
        ),
        Extension(
            "miasm.jitter.arch.JitCore_msp430",
            [*core_common, source_path("miasm/jitter/arch/JitCore_msp430.c")],
        ),
        Extension(
            ...
            [
                source_path("miasm/jitter/JitCore.c"),
                *vm_common,
                source_path("miasm/jitter/arch/JitCore_mep.c"),
            ],
        ),
        Extension(
            "miasm.jitter.arch.JitCore_mips32",
            [*core_common, source_path("miasm/jitter/arch/JitCore_mips32.c")],
        ),
        Extension(
            "miasm.jitter.arch.JitCore_ppc32",
            [*core_common, source_path("miasm/jitter/arch/JitCore_ppc32.c")],
        ),
        Extension(
            "miasm.jitter.arch.JitCore_m68k",
            [*core_common, source_path("miasm/jitter/arch/JitCore_m68k.c")],
        ),
        Extension(
            "miasm.jitter.Jitllvm",
            [
                source_path("miasm/jitter/Jitllvm.c"),
                source_path("miasm/jitter/bn.c"),
                source_path("miasm/runtime/udivmodti4.c"),
                source_path("miasm/runtime/divti3.c"),
                source_path("miasm/runtime/udivti3.c"),
            ],
        ),
        Extension(
            "miasm.jitter.Jitgcc",
            [source_path("miasm/jitter/Jitgcc.c"), source_path("miasm/jitter/bn.c")],
        ),
    ]
...
def configured_ext_modules():
    ...
    if is_win:
        ...
        if cl is None or link is None:
            BUILD_WARNINGS.append(
                ...
                "miasm-jit from a Visual Studio command prompt"
            )
    ...
    if not build_extensions:
        print("ERROR: miasm-jit extensions could not be configured, details:")
        for warning in BUILD_WARNINGS:
            print("  " + warning)
        raise RuntimeError("Unable to build miasm-jit native extensions")

    return ext_modules


class MiasmJitSdist(sdist):
    SOURCE_GLOBS = [
        "miasm/jitter/*.c",
        "miasm/jitter/*.h",
        "miasm/jitter/arch/*.c",
    ...
    def make_release_tree(self, base_dir, files):
        ...
        for pattern in self.SOURCE_GLOBS:
            for path in REPO_ROOT.glob(pattern):
                dst = Path(base_dir) / path.relative_to(REPO_ROOT)
